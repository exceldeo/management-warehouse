...
-- Dumping database structure for management-warehouse
CREATE DATABASE IF NOT EXISTS `management-warehouse` /*!40100 DEFAULT CHARACTER SET latin1 */;
USE `management-warehouse`;
...
-- Dumping data for table management-warehouse.tbl_barang: ~1 rows (approximately)
INSERT INTO `tbl_barang` (`id_barang`, `nama_barang`, `jenis`, `stok_minimum`, `stok`, `satuan`, `foto`) VALUES
	('B0001', 'minyak sanko', 2, 0, 0, 2, NULL);
...
CREATE TABLE IF NOT EXISTS `tbl_barang_masuk` (
  ...
  PRIMARY KEY (`id_transaksi`),
  KEY `FK_tbl_barang_masuk_tbl_nota` (`id_nota`),
  CONSTRAINT `FK_tbl_barang_masuk_tbl_nota` FOREIGN KEY (`id_nota`) REFERENCES `tbl_nota` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

-- Dumping data for table management-warehouse.tbl_barang_masuk: ~0 rows (approximately)

-- Dumping structure for table management-warehouse.tbl_bayar_nota
CREATE TABLE IF NOT EXISTS `tbl_bayar_nota` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `id_nota` int(11) NOT NULL DEFAULT '0',
  `tanggal_bayar` date NOT NULL,
  `jumlah_bayar` bigint(20) NOT NULL DEFAULT '0',
  `deskripsi` text,
  PRIMARY KEY (`id`),
  KEY `FK_tbl_bayar_nota_tbl_nota` (`id_nota`),
  CONSTRAINT `FK_tbl_bayar_nota_tbl_nota` FOREIGN KEY (`id_nota`) REFERENCES `tbl_nota` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=latin1;

-- Dumping data for table management-warehouse.tbl_bayar_nota: ~0 rows (approximately)
...
-- Dumping data for table management-warehouse.tbl_jenis: ~2 rows (approximately)
INSERT INTO `tbl_jenis` (`id_jenis`, `nama_jenis`) VALUES
	(1, 'elektronik'),
	(2, 'sembako');

-- Dumping structure for table management-warehouse.tbl_nota
CREATE TABLE IF NOT EXISTS `tbl_nota` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `nama_perusahaan` varchar(255) NOT NULL DEFAULT '0',
...
-- Dumping data for table management-warehouse.tbl_nota: ~0 rows (approximately)
...
-- Dumping data for table management-warehouse.tbl_satuan: ~2 rows (approximately)
